...
#
from gnuradio import gr, gru, blocks  # , uhd
import time, math, os
import juha
...
class chirprec(gr.top_block):
	def __init__(self, sr, n, op):
		...
		self.fi = blocks.file_source(gr.sizeof_gr_complex, '../../data/target.out')
		...
		if chirp_config.whiten == True:
			...
			self.connect(self.fi, self.whiten)
		...
		for i in range(0, n):
			...
			if chirp_config.whiten == True:
				...
				self.connect(self.fi, cdc)
		print('Reached end of constructor')
...
if __name__ == "__main__":
	...
	parser = OptionParser()
	parser.add_option("-o", "--outputdir", dest="outputdir", type="string", default="../../data",
	...
	while running:
		for si in range(0, s.nsounders()):
			if cr.is_done(si):
				...
				print("%s Thread %d, Next chirp %s, At %s %d:%1.3f %d %1.1f" % (
					fname, si, chirp['name'], strftime("%a, %d, %b, %Y, %H:%M:%S", time.gmtime(tnext)), chirp['rep'],
					chirp['chirpt'], overruns, cr.get_lag()))
				log.log("%s Thread %d, Next chirp %s %s %d:%1.3f %d %1.1f" % (
					fname, si, chirp['name'], strftime("%a, %d, %b, %Y, %H:%M:%S", time.gmtime(tnext)), chirp['rep'],
					chirp['chirpt'], overruns, cr.get_lag()))
